...
import SellerNetMiscCostsSection from "../forms/SellerNetMiscCostsSection";
...
export default function SellerNetSheet() {
  ...
  const [newStatement, setNewStatement] = useState<SellerNetSheetStatement>(
    defaultSellerNetSheetStatement,
  );
  ...
  useEffect(() => {
    ...
    setNewStatement(defaultSellerNetSheetStatement);
  }, [location.state]);
  ...
  const handleSubmit = () => {
    ...
    StatementApi.create({
      ...
      ...userInfo,
      endorsements: [],
    } as SellerNetSheetStatementRequest)
      .then(() => {
        navigate("/done");
      })
      .catch((err) => {
  ...
  return (
    <>
      <FormHeader title={"Seller Net Sheet"} />
      <Container
        style={{
          display: "flex",
